...
if [ -z "$DB_PASSWORD" ]; then
    ...
    AGENT_COUNT=$(PGPASSWORD="$DB_PASSWORD" psql -h "${DB_HOST:-aws-0-us-east-2.pooler.supabase.com}" -U "${DB_USER:?ERROR: DB_USER env var is required}" -d "${DB_NAME:-postgres}" -t -c "SELECT COUNT(*) FROM agents WHERE enabled = true;" 2>/dev/null | tr -d ' ')
...
if [ -z "$DB_PASSWORD" ]; then
    ...
    TABLE_COUNT=$(PGPASSWORD="$DB_PASSWORD" psql -h "${DB_HOST:-aws-0-us-east-2.pooler.supabase.com}" -U "${DB_USER:?ERROR: DB_USER env var is required}" -d "${DB_NAME:-postgres}" -t -c "SELECT COUNT(*) FROM information_schema.tables WHERE table_name IN ('agents', 'agent_executions', 'unified_memory');" 2>/dev/null | tr -d ' ')
