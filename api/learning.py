...
async def approve_proposal(
    proposal_id: str, request: ApprovalRequest = Body(default=ApprovalRequest())
) -> dict[str, Any]:
    """
    ...
    try:
        ...
        if not success:
            raise HTTPException(
                status_code=404, detail=f"Proposal {proposal_id} not found or already processed"
            )
...
async def reject_proposal(proposal_id: str, request: RejectionRequest) -> dict[str, Any]:
    """
    Reject a proposal.
...
async def run_feedback_cycle(
    ...
    try:
        ...
        return {"success": True, "cycle_results": results}
    ...
    try:
        ...
        return {
            ...
            "proposals": {
                ...
                "latest_proposal": stats["latest_proposal"].isoformat()
                if stats and stats["latest_proposal"]
                else None,
            },
            "patterns": {
                "total": pattern_stats["total_patterns"] if pattern_stats else 0,
                "types": pattern_stats["pattern_types"] if pattern_stats else 0,
                "latest": pattern_stats["latest_pattern"].isoformat()
                if pattern_stats and pattern_stats["latest_pattern"]
                else None,
            },
            "last_cycle": {
                "summary": recent_cycle["insight"] if recent_cycle else None,
                "details": recent_cycle["metadata"] if recent_cycle else None,
                "timestamp": recent_cycle["created_at"].isoformat()
                if recent_cycle and recent_cycle["created_at"]
                else None,
            },
            "retrieved_at": datetime.now(timezone.utc).isoformat(),
        }
    ...
    try:
        ...
        return {
            "overall": {
                ...
                "date_range": {
                    "earliest": overall["earliest"].isoformat()
                    if overall and overall["earliest"]
                    else None,
                    "latest": overall["latest"].isoformat()
                    if overall and overall["latest"]
                    else None,
                },
            },
            "by_type": [
