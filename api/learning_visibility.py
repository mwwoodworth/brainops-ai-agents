...
async def _get_bridge():
    ...
    if _bridge is None:
        try:
            ...
            raise HTTPException(
                status_code=503,
                detail=f"Learning-Action Bridge not available: {str(e)}"
            ) from e
    return _bridge
...
async def get_learning_bridge_status() -> dict[str, Any]:
    ...
    try:
        ...
        high_confidence_rules = sum(
            1 for r in bridge.behavior_rules.values()
            if r.confidence >= 0.8
        )
    ...
    try:
        ...
        for rule in bridge.behavior_rules.values():
            ...
            if total_applications >= 3:
                success_rate = rule.success_count / total_applications if total_applications > 0 else 0
                rule_data = {
                    "id": rule.id,
                    "rule_type": rule.rule_type.value,
                    "trigger": rule.trigger[:50] + "..." if len(rule.trigger) > 50 else rule.trigger,
                    "confidence": round(rule.confidence, 3),
...
def _generate_recommendations(status: dict, high_performers: list, low_performers: list) -> list[str]:
    """Generate actionable recommendations based on learning state"""
    recommendations = []

    if status['total_rules'] == 0:
        recommendations.append("No behavior rules yet. Trigger a sync to convert learning insights to rules.")

    if status['last_sync'] is None:
        recommendations.append("Learning-action bridge has never synced. Run /sync to pull insights.")
    ...
    if status['average_confidence'] < 0.5:
        recommendations.append("Average rule confidence is low. Consider pruning underperforming rules.")

    if len(low_performers) > len(high_performers):
        recommendations.append("More low-performing rules than high-performing. Review and prune poor rules.")

    if status['rules_applied'] == 0:
        recommendations.append("No rules have been applied yet. Ensure agents call /guidance endpoint.")
