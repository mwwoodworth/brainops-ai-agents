                    ...
                    if lock_acquired:
                        try:
                            ...
                            try:
                                ...
                                logger.warning("TaskQueueConsumer: failed to release advisory lock: %s", unlock_exc)

                if not lock_acquired:
                    # Another instance is processing; wait and retry without holding a connection.
                    logger.debug("TaskQueueConsumer: Another instance holds the lock, waiting...")
                    await asyncio.sleep(5)
                    continue

                if tasks:
                    logger.info("📥 Fetched %s pending tasks", len(tasks))
                    for task in tasks:
